import React, {useState, useEffect, useContext} from 'react'// {useState, useRef, useEffect} from 'react'
import {Link, Redirect} from 'react-router-dom'
import useFetch from 'hooks/useFetch'
import useLocalStorage from 'hooks/useLocalStorage'
import {CurrentUserContext} from 'contexts/currentUser'
import BackendErrorMessages from 'pages/authentification/components/backendErrorMessages'
...
const Autentification = props => {
    ...
    const [, setToken] = useLocalStorage('token')//мы можем теперь общаться с LS так же, как и с обычными переменными
                                                       //теперь мы можем читать напрямую с UI 
    const [currentUserState, setCurrentUserState] = useContext(CurrentUserContext)//позволяет только читать контекст и подписываться на его изменения. 
    ...
    useEffect(() => {
       ...
       // browserHistory.push('/')// вернуть на гоавную после того как зарег-сь
       //когда статус изм-ся мы хотим изменить локацию, но эот подхож не декларативенб поэтому через редирект
